...
# Import configuration to ensure consistency
try:
...
# Define the get_current_user function consistently with main app
def get_current_user_from_routes(
    ...
    # First, verify that token exists
    if not token:
    ...
    try:
        ...
        # Refresh the engine to handle connection issues
...
async def chat(
    message: str,
    current_user: User = Depends(get_current_user_from_routes),
    ...
    if not groq_client:
        # If no Groq API key is configured, provide a fallback response for task-related requests
        # This handles cases where the service is deployed without Groq API access
        logger.warning("Groq client not available, attempting to process message locally")

        # For task-related messages, try to interpret them directly
        message_lower = message.lower()
        if "add" in message_lower and ("task" in message_lower or "buy groceries" in message_lower):
            # Simulate adding a task
            try:
                # Parse the task from the message
                if "buy groceries" in message_lower:
                    title = "Buy groceries"
                    description = "Grocery shopping task"
                else:
                    # Extract task title from message
                    title = message.replace("Add a new task:", "").replace("add", "").strip()
                    description = f"Task created from message: {message}"

                # Add the task directly using MCP
                ...
                if tool_result and tool_result.get("success"):
                    ...
                    response_msg = f"I've added the task: '{task.get('title', title)}'. You can view your tasks to confirm."
                else:
                    response_msg = "I tried to add your task, but there was an issue. Please try again or use the task form directly."
                ...
                logger.error(f"Error processing task locally: {str(e)}")
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                    detail="Unable to process your request. Groq API is not configured and local processing failed."
                )
        else:
            # For non-task messages when Groq is not available
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
